...
import { DuidAveragePriceDatum } from './plugin/transformProps';
...
export function transformData(data: DuidAveragePriceDatum[]) {
...
export function getPeriods(data: DuidAveragePriceDatum[]) {
  const orderedData = data.sort((a, b) =>
    (a['`Year`'] as string) > (b['`Year`'] as string) ? 1 : -1,
  );
